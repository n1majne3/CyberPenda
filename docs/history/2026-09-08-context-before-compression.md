# Domain documentation snapshot — 2026-09-08

Status: historical reference, not current instructions or a second glossary.

This is the exact pre-compression CONTEXT.md, including accumulated decisions,
implementation notes, and superseded statements. Some entries describe current
behavior; others describe retired behavior. Do not apply it as a whole.
Use the [current glossary](../../CONTEXT.md) for terms and the
[current product boundary](../product/current-boundary.md) for supported behavior.
For a specific older constraint, compare the relevant ADR, current specification,
and code before use. Read this snapshot only when tracing a prior decision.
...
retired. Existing Hermes Profiles, Runtime Configuration Snapshots, and
Transcript records remain readable. New launches and Resume cannot execute
Hermes and never substitute a different Runtime. User Runtime homes remain
untouched. This decision supersedes the Hermes execution and packaging parts
...
The **Config Projection** that makes every global **Launch-Ready Model Provider**, its model configuration, and its configured model API credential available to a **Runtime** that can switch providers natively. Pi uses it. Codex and Claude Code do not.
...
The on-demand, redacted provider-native configuration generated by the daemon from a saved **Runtime Profile**, including its **Custom Config File**. Task-specific values are added at launch. The UI does not generate a separate preview or substitute one when projection fails. Editing and **Profile Config Import** share this view; unsaved form changes must be saved before import.
...
- A **Model Provider Migration** presents a **Model Provider Migration Preview** before creating or updating a **Model Provider**. Its settings panel is visible only when the preview reports eligible legacy data.
...
- Every Pi task uses **Global Model Projection** rather than limiting model configuration or credentials to its initially selected **Model Provider**.
- A Pi **Runtime Turn** switches **Model Provider**, model, and **Reasoning Effort** through runtime-native controls without restarting the **Runtime** when those controls exist; otherwise the Harness restarts the **Runtime**.
...
- Built-in Codex, Claude Code, and Pi **Runtimes** use a **Runtime Owner-Scoped Persistent Runtime** on both **Sandbox Runner** and **Host Runner** when their native session bridge is available.
...
- The **TSecBench Hosted Image** is not limited to one Runtime family; resolved: it packages Pi, Codex and Claude Code.
...
- Runtime family selection is explicit; resolved: Codex is the default, Hosted bootstrap accepts Codex, Claude Code, or Pi with a compatible model protocol and rejects Hermes as the selected Runtime. These three Runtime CLIs remain in the **Hosted Tool Baseline**.
...
- Hosted Runtime dependencies are not pinned at source level; resolved: builds install current Pi, Codex and Claude Code releases, and each produced archive records the exact resolved versions for traceability.
