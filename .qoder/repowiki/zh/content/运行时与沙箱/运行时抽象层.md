...
## 更新摘要
**变更内容**   
- 新增 Claude、Codex、Pi 三个辅助运行时实现，提供增强的会话管理和验证能力
- 扩展 ProviderSession 接口以支持更丰富的交互模式
- 增强会话状态机处理，支持中断替换和权限响应机制
- 改进错误处理和超时控制机制
- 优化容器化执行抽象和资源清理流程
...
**更新** 本次更新重点介绍了新增的 Claude、Codex、Pi 辅助运行时实现，这些实现提供了增强的会话管理和验证能力，进一步完善了运行时抽象层的完整性和可扩展性。
...
CA["ClaudeAssisted<br/>Claude 辅助运行时"]
CO["CodexAssisted<br/>Codex 辅助运行时"]
PI["PiAssisted<br/>Pi 辅助运行时"]
end
subgraph "Provider"
Proc["Provider 进程/容器"]
end
...
- [claude_assisted.go:1-200](file://internal/runtime/claude_assisted.go#L1-L200)
- [codex_assisted.go:1-200](file://internal/runtime/codex_assisted.go#L1-L200)
- [pi_assisted.go:1-200](file://internal/runtime/pi_assisted.go#L1-L200)
...
- **新增** ClaudeAssisted/CodexAssisted/PiAssisted：针对特定 AI 提供商的辅助运行时实现，提供优化的会话管理和验证逻辑。
...
participant RA as "辅助运行时(Claude/Codex/Pi)"
...
- [provider_adapters.go:126-200](file://internal/runtime/provider_adapters.go#L126-200)
- [claude_assisted.go:100-150](file://internal/runtime/claude_assisted.go#L100-L150)
- [codex_assisted.go:100-150](file://internal/runtime/codex_assisted.go#L100-L150)
- [pi_assisted.go:100-150](file://internal/runtime/pi_assisted.go#L100-L150)
- [session_bridge.go:379-442](file://internal/runtime/session_bridge.go#L379-L442)
- [host_session_bridge.go:252-315](file://internal/runtime/host_session_bridge.go#L252-315)
- [docker_sandbox.go:111-231](file://internal/runtime/docker_sandbox.go#L111-231)
...
- **新增** 具体 Provider 适配器（Codex/Claude/Pi）通过 providerWireMethods 配置 wire 方法名、参数构造、turnID/sessionID 提取以及可选 prepareSend（如 Pi 的 set_model → set_thinking_level → prompt）。
...
providerSessionAdapter <|-- ClaudeAssisted
providerSessionAdapter <|-- CodexAssisted
providerSessionAdapter <|-- PiAssisted
```
...
- [claude_assisted.go:50-120](file://internal/runtime/claude_assisted.go#L50-L120)
- [codex_assisted.go:50-120](file://internal/runtime/codex_assisted.go#L50-L120)
- [pi_assisted.go:50-120](file://internal/runtime/pi_assisted.go#L50-L120)
...
#### Claude 辅助运行时
ClaudeAssisted 实现了针对 Claude API 的专门优化：
- 增强的会话验证机制，确保连接稳定性
- 智能重试策略，处理网络波动和临时错误
- 优化的消息格式转换，提高通信效率
- 详细的错误分类和恢复策略
...
#### Codex 辅助运行时  
CodexAssisted 提供了 Codex CLI 的专用适配：
- 命令行参数动态生成和优化
- 实时输出解析和事件转换
- 进程生命周期管理和资源清理
- 环境变量和配置文件的自动处理
...
- 资源清理：
  ...
  - **新增** 辅助运行时清理：Claude/Codex/Pi 各自实现特定的资源清理逻辑，确保会话完全终止。
...
- [claude_assisted.go:150-200](file://internal/runtime/claude_assisted.go#L150-L200)
- [codex_assisted.go:150-200](file://internal/runtime/codex_assisted.go#L150-L200)
- [pi_assisted.go:150-200](file://internal/runtime/pi_assisted.go#L150-L200)
...
- **新增** 辅助运行时依赖各自的 Provider SDK 和验证库，提供增强的功能支持。
...
ProviderSession --> ClaudeAssisted["ClaudeAssisted"]
ProviderSession --> CodexAssisted["CodexAssisted"]
ProviderSession --> PiAssisted["PiAssisted"]
ClaudeAssisted --> ClaudeSDK["Claude SDK"]
CodexAssisted --> CodexCLI["Codex CLI"]
PiAssisted --> PiPlatform["Pi Platform"]
```
...
- [claude_assisted.go:120-200](file://internal/runtime/claude_assisted.go#L120-L200)
- [codex_assisted.go:120-200](file://internal/runtime/codex_assisted.go#L120-L200)
- [pi_assisted.go:120-200](file://internal/runtime/pi_assisted.go#L120-L200)
...
- 常见错误类型：
  ...
  - **新增** 辅助运行时特定错误：Claude/Codex/Pi 各自定义了专门的错误类型，便于问题定位。
- 定位步骤：
  ...
  - **新增** 辅助运行时调试：检查各辅助运行时的初始化日志和验证结果。
- 恢复策略：
  ...
  - **新增** 辅助运行时恢复：各辅助运行时提供特定的恢复机制，确保会话一致性。
...
**更新** 新增的 Claude、Codex、Pi 辅助运行时实现进一步增强了系统的完整性和可靠性，为不同类型的 AI 提供商提供了专门优化的支持，使得运行时抽象层能够更好地适应多样化的 AI 服务需求。
...
- 新增 Provider 的步骤：
  ...
  2. **新增** 实现辅助运行时类（参考 ClaudeAssisted/CodexAssisted/PiAssisted）：
     - Initialize() 方法：初始化连接和验证环境。
     - ValidateSession() 方法：验证会话有效性和连接状态。
     - EnhancedValidation() 方法：提供额外的验证逻辑。
     - 特定的错误处理和恢复策略。
  3. 若需要新的传输方式，实现 ProviderSessionTransport（Send/Close）。
  4. 在 ProductionFactory 中注册该 Provider 的桥接策略（容器/宿主）。
  5. 通过 Capability 声明支持的交互能力（persistent_session/send_turn/interrupt_turn/in_turn_steer/permission_response/resume_session）。
  6. 编写测试覆盖：
     - 能力检查与不支持错误。
     - 请求幂等与冲突检测。
     - settlement 等待与超时。
...
- 最佳实践：
  ...
  - **新增** 为辅助运行时实现适当的日志记录和错误分类，便于问题诊断。
  - **新增** 考虑不同 Provider 的网络特性和 API 限制，实现针对性的超时和重试策略。
