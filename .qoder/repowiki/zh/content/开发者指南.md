...
- [internal/daemon/resume_harness_wait_test.go](file://internal/daemon/resume_harness_wait_test.go)
- [internal/daemon/runtime_activity.go](file://internal/daemon/runtime_activity.go)
- [internal/daemon/provider_session_launch_test.go](file://internal/daemon/provider_session_launch_test.go)
</cite>

## 更新摘要
**变更内容**   
- 增强了守护进程异步状态管理的测试可靠性实践，包括适当的等待机制和轮询模式处理竞态条件
- 新增了运行时活动监控和超时控制机制
- 改进了任务恢复流程中的竞态条件处理
- 更新了测试辅助函数以支持更可靠的异步状态验证
...
D["internal/daemon/runtime_activity.go"]
end
subgraph "领域服务"
E["internal/blackboardv2/service.go"]
F["internal/modelprovider/..."]
G["internal/skill/..."]
H["internal/task/..."]
end
subgraph "运行时与沙箱"
I["internal/runtime/runtime.go"]
J["internal/runtime/docker_sandbox.go"]
K["internal/runtime/session_bridge.go"]
end
subgraph "前端"
L["web/src/pages/RuntimeProfilesPage.tsx"]
end
A --> C
C --> D
C --> E
...
- [internal/daemon/runtime_activity.go:218-232](file://internal/daemon/runtime_activity.go#L218-L232)
...
participant Activity as "运行时活动监控"
...
Daemon->>Activity : "检查运行时活动状态"
Activity-->>Daemon : "返回活跃性信息"
...
### 运行时活动监控与异步状态管理
**新增** 增强了异步状态管理的测试可靠性，包括适当的等待机制和轮询模式处理竞态条件。

- 运行时活动监控：提供任务活跃性、轮次活动和警告信息的实时查询
- 超时控制机制：可配置的 runtimeStopTimeout 用于控制等待时间
- 轮询策略：自适应间隔轮询，短超时情况下提高轮询频率
- 竞态条件处理：确保任务恢复时正确处理活跃的 harness 状态
...
-runtimeStopTimeout
-harness
+runtimeHarnessWaitTimeout() time.Duration
+waitRuntimeHarnessInactive(taskID) bool
+waitHarnessInactive(taskID, timeout) bool
+ensureRuntimeAbsentBeforeLaunch(taskID) error
}
class RuntimeActivity {
+liveness
+turn_activity
+warning
+computeRuntimeActivity(task) RuntimeActivity
+reconcileRuntimeActivity(task, activity) (task, RuntimeActivity)
}
class TestHelpers {
+waitForHarnessActive(t, server, taskID, want) void
}
Server --> RuntimeActivity : "计算和协调"
Server --> TestHelpers : "测试辅助"
```

图表来源
- [internal/daemon/runtime_activity.go:218-232](file://internal/daemon/runtime_activity.go#L218-L232)
- [internal/daemon/runtime_activity.go:234-259](file://internal/daemon/runtime_activity.go#L234-L259)
- [internal/daemon/provider_session_launch_test.go:279-289](file://internal/daemon/provider_session_launch_test.go#L279-L289)
...
### 任务恢复与竞态条件处理
**新增** 改进了任务恢复流程中的竞态条件处理，确保正确的等待机制。

- 恢复等待机制：POST /resume 必须等待旧的 harness 释放后才能接受新的运行时
- 超时处理：当 harness 保持活跃时返回稳定的 409 冲突错误
- 状态一致性：确保不会在等待期间创建新的 Continuation 或绑定 Provider Session
- 测试覆盖：完整的竞态条件测试场景，包括正常释放和超时情况

```mermaid
sequenceDiagram
participant Client as "客户端"
participant Server as "守护进程"
participant Harness as "运行时 Harness"
participant Factory as "会话工厂"
Client->>Server : "POST /resume"
Server->>Harness : "检查 IsActive()"
alt harness 仍活跃
Server->>Server : "等待直到超时"
Server-->>Client : "409 Conflict"
else harness 已释放
Server->>Factory : "创建新会话"
Factory-->>Server : "返回新会话"
Server-->>Client : "202 Accepted"
end
```

图表来源
- [internal/daemon/resume_harness_wait_test.go:89-157](file://internal/daemon/resume_harness_wait_test.go#L89-L157)
- [internal/daemon/resume_harness_wait_test.go:159-216](file://internal/daemon/resume_harness_wait_test.go#L159-L216)
...
- [internal/runtime/runtime.go:333-481](file://internal/runtime/runtime.go#L333-481)
- [internal/runtime/docker_sandbox.go:20-57](file://internal/runtime/docker_sandbox.go#L20-L57)
...
- **新增** 任务恢复超时：检查 runtimeStopTimeout 配置和 harness 状态，确认是否有活跃的运行时未正确释放。
- **新增** 异步状态不一致：使用 waitForHarnessActive 辅助函数验证状态，检查轮询间隔和超时配置。
...
## 结论
本指南梳理了 CyberPenda 的核心子系统与贡献要点。遵循 Blackboard v2 的语义契约、守护进程的安全边界与运行时隔离原则，结合完善的测试与构建流程，可高效推进功能开发与质量保障。**新增的异步状态管理改进显著提升了测试可靠性和生产环境的稳定性**。
...
- [internal/daemon/server.go:383-411](file://internal/daemon/server.go#L383-L411)
- [internal/daemon/server.go:518-534](file://internal/daemon/server.go#L518-L534)
- [internal/blackboardv2/service.go:616-630](file://internal/blackboardv2/service.go#L616-630)
- [internal/daemon/runtime_activity.go:218-232](file://internal/daemon/runtime_activity.go#L218-L232)
...
- **新增** 异步状态测试最佳实践
  - 使用 waitForHarnessActive 辅助函数验证状态
  - 实现可配置的超时控制（server.runtimeStopTimeout）
  - 覆盖竞态条件场景（正常释放、超时、重复请求）
  - 确保测试的可重现性和稳定性
...
E --> F["CI 流水线触发"]
F --> G["异步状态验证(waitForHarnessActive)"]
G --> H["超时控制(runtimeStopTimeout)"]
```
...
- 评审要点
  ...
  - **新增** 异步代码审查：检查等待机制、超时处理、竞态条件处理和测试覆盖。
- 合并
  - 通过 CI 测试与冒烟用例后再合并。

章节来源
- [Makefile:73-76](file://Makefile#L73-76)
- [internal/daemon/server.go:383-411](file://internal/daemon/server.go#L383-L411)
