...
- [provider_session_launch_test.go](file://internal/daemon/provider_session_launch_test.go)
...
## 更新摘要
**所做更改**   
- 增强了测试可靠性：为守护进程任务恢复功能添加了轮询机制，处理Harness注册与持久化状态写入之间的竞态条件
- 改进了任务状态同步：确保在测试断言之前任务达到运行状态
- 优化了等待超时机制：使用可配置的runtimeStopTimeout替代硬编码常量
- 增强了并发安全性：防止在旧Harness释放前启动新的Runtime实例
...
- 运行时活动管理：runtime_activity.go
- 运行时与测试：runtime_turn_selection_test.go、resume_harness_wait_test.go、finish_task_resume_test.go、blackboard_v2_continuity_test.go、blackboard_v2_cli_test.go
...
- [runtime_activity.go:219-286](file://internal/daemon/runtime_activity.go#L219-L286)
- [resume_harness_wait_test.go:137-167](file://internal/daemon/resume_harness_wait_test.go#L137-L167)
- [finish_task_resume_test.go:745-760](file://internal/daemon/finish_task_resume_test.go#L745-L760)
...
- **新增**：运行时活动管理：提供增强的轮询机制和超时处理，确保任务状态同步的可靠性
...
- [task_handlers.go:1815-1912](file://internal/daemon/task_handlers.go#L1815-L1912)
- [runtime_activity.go:228-259](file://internal/daemon/runtime_activity.go#L228-L259)
- [blackboard_v2_http.go:224-236](file://internal/daemon/blackboard_v2_http.go#L224-L236)
...
**章节来源**
- [continuity.go:764-880](file://internal/blackboardv2/continuity.go#L764-L880)
- [continuity.go:941-976](file://internal/blackboardv2/continuity.go#L941-976)
...
### 增强的测试可靠性与竞态条件处理
**新增** 针对守护进程任务恢复功能的测试可靠性增强，主要改进包括：

- **轮询机制**：在测试中添加轮询逻辑，确保任务达到运行状态后再进行断言，避免Harness注册与持久化状态写入之间的竞态条件
- **可配置超时**：使用 `server.runtimeStopTimeout` 替代硬编码常量，提供更灵活的超时控制
- **状态同步验证**：通过 `waitForHarnessActive` 函数确保Harness状态与实际运行状态同步
- **并发安全**：防止在旧Harness释放前启动新的Runtime实例，确保任务恢复的原子性

```mermaid
sequenceDiagram
participant Test as "测试用例"
participant Server as "守护进程服务器"
participant Harness as "运行时Harness"
participant TaskStore as "任务存储"
Test->>Server : POST /tasks/{id}/resume
Server->>Harness : 检查IsActive(taskID)
alt Harness仍活跃
Server->>Server : waitRuntimeHarnessInactive(timeout)
loop 轮询等待
Server->>Harness : IsActive(taskID)?
alt 已释放
break
end
Server->>Server : sleep(interval)
end
else Harness已释放
Server->>Server : 继续处理恢复
end
Server->>TaskStore : 更新任务状态为running
Test->>TaskStore : 轮询直到status=running
TaskStore-->>Test : 返回running状态
Test->>Test : 执行断言验证
```

**图表来源**
- [resume_harness_wait_test.go:89-157](file://internal/daemon/resume_harness_wait_test.go#L89-L157)
- [finish_task_resume_test.go:745-760](file://internal/daemon/finish_task_resume_test.go#L745-L760)
- [runtime_activity.go:228-259](file://internal/daemon/runtime_activity.go#L228-L259)
- [provider_session_launch_test.go:270-282](file://internal/daemon/provider_session_launch_test.go#L270-L282)
...
- **新增**：运行时活动管理模块提供增强的轮询和超时处理，确保任务状态同步的可靠性
...
- **新增**：轮询间隔优化：使用自适应轮询间隔（默认5ms），根据超时时间动态调整，确保短超时测试仍能观察到状态变化

**章节来源**
- [health.go:294-309](file://internal/blackboardv2/health.go#L294-L309)
- [continuity.go:1095-1147](file://internal/blackboardv2/continuity.go#L1095-L1147)
- [continuity.go:227-325](file://internal/blackboardv2/continuity.go#L227-L325)
- [runtime_activity.go:244-251](file://internal/daemon/runtime_activity.go#L244-L251)
...
- 常见错误码
  ...
  - **新增**：runtime harness is still active：旧Harness仍在活跃状态，需要等待释放或超时
- 典型问题定位
  ...
  - **新增**：任务状态不同步：检查轮询机制是否正常工作，确认runtimeStopTimeout设置是否合理
...
- [blackboard_v2_continuity_test.go:370-437](file://internal/daemon/blackboard_v2_continuity_test.go#L370-L437)
- [task_handlers.go:1843-1848](file://internal/daemon/task_handlers.go#L1843-L1848)
...
**新增**的测试可靠性增强通过轮询机制和可配置超时处理，有效解决了Harness注册与持久化状态写入之间的竞态条件，确保任务恢复过程的稳定性和可预测性。这些改进显著提升了系统的健壮性和测试的可重复性。
...
- **新增**：运行时活动管理
  - waitRuntimeHarnessInactive：等待Harness释放，支持可配置超时
  - ensureRuntimeAbsentBeforeLaunch：在启动新Runtime前清理或证明旧Runtime不存在
  - runtimeHarnessWaitTimeout：获取运行时等待超时时间
- 前端契约
