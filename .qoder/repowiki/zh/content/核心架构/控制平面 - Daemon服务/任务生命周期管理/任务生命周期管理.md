# 任务生命周期管理

<cite>
**本文引用的文件**   
- [task_handlers.go](file://internal/daemon/task_handlers.go)
- [provider_session_control.go](file://internal/daemon/provider_session_control.go)
- [continuity.go](file://internal/blackboardv2/continuity.go)
- [provider_session.go](file://internal/runtime/provider_session.go)
- [runtime.go](file://internal/runtime/runtime.go)
- [task.go](file://internal/task/task.go)
- [blackboard_v2_continuity_test.go](file://internal/daemon/blackboard_v2_continuity_test.go)
- [finish_task_resume_test.go](file://internal/daemon/finish_task_resume_test.go)
- [runtime_turn_selection_test.go](file://internal/daemon/runtime_turn_selection_test.go)
- [smoke-runtime-tasks-live.py](file://scripts/smoke-runtime-tasks-live.py)
- [assisted_conclusion_control_test.go](file://internal/daemon/assisted_conclusion_control_test.go)
- [assisted_conclusion_terminal_test.go](file://internal/daemon/assisted_conclusion_terminal_test.go)
- [assisted_conclusion_test.go](file://internal/daemon/assisted_conclusion_test.go)
- [blackboard_conclusion.go](file://internal/daemon/blackboard_conclusion.go)
- [blackboard_conclusion_recovery.go](file://internal/daemon/blackboard_conclusion_recovery.go)
- [result.go](file://internal/blackboardconclusion/result.go)
- [result_service_test.go](file://internal/blackboardconclusion/result_service_test.go)
</cite>

## 更新摘要
**所做更改**   
- 新增助手结论支持章节，详细说明 Assisted Conclusion 机制和终端处理流程
- 增强 Blackboard 验证功能部分，包含结果服务和恢复逻辑
- 更新任务处理器相关章节，反映新的助手结论集成
- 扩展 API 参考，包含助手结论相关的端点
- 更新故障排查指南，包含助手结论相关的错误处理

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能与可靠性](#性能与可靠性)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录：API参考与使用示例](#附录api参考与使用示例)

## 简介
本文件面向本地优先的渗透测试代理系统，聚焦"任务生命周期管理"的完整闭环：创建、启动、监控、停止、恢复；并深入解析状态机转换、事件驱动架构、进度跟踪与日志收集机制。文档同时覆盖 Provider 会话控制、权限响应处理、Steer 指令队列与 Continuation 恢复逻辑，并提供完整的 API 参考与实际使用场景示例。

**更新** 本次更新重点增强了助手结论（Assisted Conclusion）支持和 Blackboard 验证能力，为任务生命周期管理提供了更完善的自动化完成机制。

## 项目结构
围绕任务生命周期的关键代码分布在以下模块：
- Daemon HTTP 服务层：任务路由、编排、事件持久化、Provider 会话注册与控制
- Blackboard v2 连续性服务：Continuation 原子创建、Working Snapshot 同步与发布、授权令牌
- Runtime/Harness：进程/容器隔离、适配器、停止与完成语义
- Task 服务：事件存储、未消费指令、Reconciliation 标记
- Provider Session：能力协商、操作模式、幂等与冲突控制
- **新增** 助手结论服务：自动化任务完成判断、Blackboard 验证、结果生成

```mermaid
graph TB
subgraph "控制平面(Daemon)"
A["HTTP 路由<br/>任务/Steer/Resume/Finish"] --> B["任务服务(Task Service)"]
A --> C["Provider 会话注册表"]
A --> D["Blackboard v2 连续性服务"]
A --> E["助手结论服务(Assisted Conclusion)"]
end
subgraph "执行平面(Runtime/Harness)"
F["适配器(命令/沙箱)"] --> G["进程/容器"]
H["Provider 会话接口"] --> I["具体实现(Fake/桥接)"]
end
subgraph "记忆平面(Blackboard v2)"
J["Launch Pin / Working Snapshot"]
K["同步与授权(Grant)"]
L["结果验证与结论"]
end
A --> F
A --> H
D --> J
D --> K
E --> L
```

**图表来源**
- [task_handlers.go:1-800](file://internal/daemon/task_handlers.go#L1-L800)
- [continuity.go:764-880](file://internal/blackboardv2/continuity.go#L764-L880)
- [provider_session.go:140-152](file://internal/runtime/provider_session.go#L140-L152)
- [runtime.go:153-189](file://internal/runtime/runtime.go#L153-L189)
- [blackboard_conclusion.go:1-100](file://internal/daemon/blackboard_conclusion.go#L1-L100)

**章节来源**
- [task_handlers.go:1-800](file://internal/daemon/task_handlers.go#L1-L800)
- [continuity.go:764-880](file://internal/blackboardv2/continuity.go#L764-L880)
- [provider_session.go:140-152](file://internal/runtime/provider_session.go#L140-L152)
- [runtime.go:153-189](file://internal/runtime/runtime.go#L153-L189)
- [blackboard_conclusion.go:1-100](file://internal/daemon/blackboard_conclusion.go#L1-L100)

## 核心组件
- 任务服务与事件总线
  - 负责任务 CRUD、事件追加、未消费 Harness Steering 查询、Continuation 状态与 Reconciliation 标记维护。
- Blackboard v2 连续性服务
  - 提供 Continuation 原子创建（含 Launch Pin、Working Snapshot、Grant）、同步与重放、活跃快照恢复。
- Provider 会话控制
  - 在内存中注册与绑定 Task 级会话，支持 SendTurn/InterruptThenReplace/InTurnSteer/PermissionResponse 等操作，具备幂等键与冲突检测。
- Runtime/Harness
  - 封装命令或沙箱适配器，统一 Stop/StopAndWait/IsActive 语义，并在退出时写入 lifecycle 事件与最终状态。
- **新增** 助手结论服务
  - 基于 Blackboard 状态自动判断任务完成情况，支持多种验证规则和结果生成策略。

**章节来源**
- [task.go:564-627](file://internal/task/task.go#L564-L627)
- [continuity.go:764-880](file://internal/blackboardv2/continuity.go#L764-L880)
- [provider_session_control.go:18-111](file://internal/daemon/provider_session_control.go#L18-L111)
- [runtime.go:153-189](file://internal/runtime/runtime.go#L153-L189)
- [result.go:1-150](file://internal/blackboardconclusion/result.go#L1-L150)

## 架构总览
下图展示一次"中断式 Steer + Native Resume"的典型流程，包括状态切换、事件流与 Continuation 替换，以及新增的助手结论处理路径。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant Daemon as "Daemon(HTTP)"
participant TaskSvc as "任务服务"
participant BBV2 as "Blackboard v2 连续性"
participant Harness as "Runtime/Harness"
participant Prov as "Provider 会话"
participant AC as "助手结论服务"
Client->>Daemon : POST /steer (directive, selection)
Daemon->>TaskSvc : AppendEvent(steering_requested)
alt 有活动会话
Daemon->>Prov : InterruptThenReplace/InTurnSteer
Prov-->>Daemon : outcome=acknowledged/settled/started
Daemon->>TaskSvc : AppendEvent(lifecycle : resuming_native)
Daemon->>BBV2 : CreateContinuation(Launch Pin/Working Snapshot)
Daemon->>Harness : Launch(new Continuation)
Harness-->>Daemon : lifecycle finish_shutdown
Daemon->>AC : 触发助手结论检查
AC-->>Daemon : 结论验证结果
Daemon->>TaskSvc : UpdateStatus/UpdateContinuationStatus
else 无活动会话
Daemon->>TaskSvc : 记录配置版本/等待下次调度
end
Daemon-->>Client : 202 Accepted(包含新 Task 详情)
```

**图表来源**
- [task_handlers.go:2176-2324](file://internal/daemon/task_handlers.go#L2176-L2324)
- [continuity.go:764-880](file://internal/blackboardv2/continuity.go#L764-L880)
- [runtime.go:153-189](file://internal/runtime/runtime.go#L153-L189)
- [blackboard_conclusion.go:100-200](file://internal/daemon/blackboard_conclusion.go#L100-L200)

## 详细组件分析

### 任务状态机与事件流
- 状态集合
  - pending、running、paused、stopped、failed、completed
- 关键转换
  - 启动：pending → running（由 Harness 启动后更新）
  - 停止：running/paused → stopped（显式 Stop 或 Finish 失败回退）
  - 失败：running/paused → failed（异常或 Finish 失败回退）
  - 完成：仅 operator_finish 路径可写 completed（Finish 成功后）
- 事件类型
  - lifecycle：process_started、interrupting、resuming_native、finish_shutdown、completed、failed、stopped 等
  - steering：steering_requested、steering_applied（含 requested_event_id）
  - conversation/steering（Provider 会话）：mode/outcome/request_id/session_id/provider_turn_id
  - runtime_output：文本输出与流式日志
  - **新增** conclusion：assisted_conclusion_triggered、validation_passed、validation_failed

```mermaid
stateDiagram-v2
[*] --> pending
pending --> running : "launch"
running --> paused : "pause"
paused --> running : "resume"
running --> stopped : "stop"
paused --> stopped : "stop"
running --> failed : "error/finish_fail"
paused --> failed : "error/finish_fail"
running --> completed : "operator_finish"
running --> completed : "assisted_conclusion"
stopped --> pending : "resume"
failed --> pending : "resume"
```

**图表来源**
- [runtime.go:153-189](file://internal/runtime/runtime.go#L153-L189)
- [task_handlers.go:1469-1551](file://internal/daemon/task_handlers.go#L1469-L1551)
- [task_handlers.go:1600-1714](file://internal/daemon/task_handlers.go#L1600-L1714)
- [blackboard_conclusion.go:200-300](file://internal/daemon/blackboard_conclusion.go#L200-L300)

**章节来源**
- [runtime.go:153-189](file://internal/runtime/runtime.go#L153-L189)
- [task_handlers.go:1469-1551](file://internal/daemon/task_handlers.go#L1469-L1551)
- [task_handlers.go:1600-1714](file://internal/daemon/task_handlers.go#L1600-L1714)
- [blackboard_conclusion.go:200-300](file://internal/daemon/blackboard_conclusion.go#L200-L300)

### 助手结论支持与 Blackboard 验证
**新增** 助手结论系统是任务生命周期管理的重要增强，提供自动化的任务完成判断和验证机制。

- 核心功能
  - 基于 Blackboard 状态自动判断任务是否已完成
  - 支持多种验证规则：目标达成、证据完整性、发现质量等
  - 异步处理，不阻塞主任务流程
  - 可配置的验证阈值和重试机制

- 工作流程
  - 任务完成后触发助手结论检查
  - 加载当前 Blackboard 状态和验证规则
  - 执行多维度验证：事实完整性、关系一致性、证据有效性
  - 根据验证结果自动生成结论报告
  - 更新任务状态为 completed 并记录审计日志

```mermaid
flowchart TD
Start(["任务完成"]) --> Trigger["触发助手结论检查"]
Trigger --> LoadState["加载 Blackboard 状态"]
LoadState --> ValidateRules["应用验证规则"]
ValidateRules --> CheckFacts{"事实完整性检查"}
CheckFacts --> |通过| CheckRelations{"关系一致性检查"}
CheckFacts --> |失败| FailConclusion["生成失败结论"]
CheckRelations --> |通过| CheckEvidence{"证据有效性检查"}
CheckRelations --> |失败| FailConclusion
CheckEvidence --> |通过| GenerateReport["生成结论报告"]
CheckEvidence --> |失败| FailConclusion
GenerateReport --> UpdateStatus["更新任务状态为 completed"]
FailConclusion --> LogFailure["记录失败原因"]
UpdateStatus --> End(["完成"])
LogFailure --> End
```

**图表来源**
- [blackboard_conclusion.go:150-250](file://internal/daemon/blackboard_conclusion.go#L150-L250)
- [result.go:100-200](file://internal/blackboardconclusion/result.go#L100-L200)
- [blackboard_conclusion_recovery.go:1-100](file://internal/daemon/blackboard_conclusion_recovery.go#L1-L100)

**章节来源**
- [blackboard_conclusion.go:150-250](file://internal/daemon/blackboard_conclusion.go#L150-L250)
- [result.go:100-200](file://internal/blackboardconclusion/result.go#L100-L200)
- [blackboard_conclusion_recovery.go:1-100](file://internal/daemon/blackboard_conclusion_recovery.go#L1-L100)

### 任务创建与启动流程
- 入口：handleCreateTask
  - 校验与默认值填充、预检（preflight）、激活校验
  - 构建 Launch Plan（含模型选择、推理强度、Skill Bundles、Layout）
  - 准备 Blackboard v2 Continuation（Precommit/BindGrant/UnbindGrant 钩子）
  - 可选打开持久化 Provider 会话（工厂模式），设置初始 Turn Selection
  - 后台 Launch，完成后根据结果清理会话并记录日志
- 关键点
  - Blackboard v2 必须启用；不支持的 Provider 直接拒绝
  - Precommit 阶段生成 Launch Header 与投影；BindGrant 注入一次性 Grant
  - 固定字段合并到 CapturedRuntimeConfig，避免敏感信息落盘
  - **新增** 启动后自动触发助手结论检查

```mermaid
flowchart TD
Start(["创建任务"]) --> Validate["参数校验/默认值/预检"]
Validate --> BuildPlan["构建 Launch Plan"]
BuildPlan --> BBPrepare["准备 Blackboard v2 Continuation"]
BBPrepare --> OpenSession{"是否持久化会话?"}
OpenSession --> |是| BindSession["绑定 Provider 会话/初始 Turn Selection"]
OpenSession --> |否| SkipSession["跳过"]
BindSession --> Launch
SkipSession --> Launch
Launch["后台 Launch(Harness)"] --> PostLaunch["启动后处理"]
PostLaunch --> AssistedCheck["触发助手结论检查"]
AssistedCheck --> Done(["返回已创建任务详情"])
```

**图表来源**
- [task_handlers.go:73-167](file://internal/daemon/task_handlers.go#L73-L167)
- [task_handlers.go:196-285](file://internal/daemon/task_handlers.go#L196-L285)
- [task_handlers.go:311-397](file://internal/daemon/task_handlers.go#L311-L397)
- [continuity.go:764-880](file://internal/blackboardv2/continuity.go#L764-L880)
- [blackboard_conclusion.go:250-350](file://internal/daemon/blackboard_conclusion.go#L250-L350)

**章节来源**
- [task_handlers.go:73-167](file://internal/daemon/task_handlers.go#L73-L167)
- [task_handlers.go:196-285](file://internal/daemon/task_handlers.go#L196-L285)
- [task_handlers.go:311-397](file://internal/daemon/task_handlers.go#L311-L397)
- [continuity.go:764-880](file://internal/blackboardv2/continuity.go#L764-L880)
- [blackboard_conclusion.go:250-350](file://internal/daemon/blackboard_conclusion.go#L250-L350)

### 监控与进度跟踪
- 事件拉取：/events、/timeline、/transcript
- 运行时活动：attachRuntimeActivity 附加健康/空闲/忙碌状态
- 轮询脚本：smoke-runtime-tasks-live.py 基于 events 与 status 判定卡住/错误/超时
- **新增** 助手结论事件监控：conclusion_triggered、validation_passed、validation_failed

```mermaid
sequenceDiagram
participant UI as "前端/CLI"
participant Daemon as "Daemon"
participant TaskSvc as "任务服务"
participant AC as "助手结论服务"
UI->>Daemon : GET /tasks/ : id/events
Daemon->>TaskSvc : Events(taskID)
TaskSvc-->>Daemon : []Event
Note over Daemon,AC : 监控助手结论事件
AC-->>Daemon : conclusion events
Daemon-->>UI : {events with conclusion data}
Note over UI,Daemon : 前端按 phase 渲染 timeline/transcript
```

**图表来源**
- [task_handlers.go:1400-1467](file://internal/daemon/task_handlers.go#L1400-L1467)
- [smoke-runtime-tasks-live.py:176-211](file://scripts/smoke-runtime-tasks-live.py#L176-L211)
- [blackboard_conclusion.go:300-400](file://internal/daemon/blackboard_conclusion.go#L300-L400)

**章节来源**
- [task_handlers.go:1400-1467](file://internal/daemon/task_handlers.go#L1400-L1467)
- [smoke-runtime-tasks-live.py:176-211](file://scripts/smoke-runtime-tasks-live.py#L176-L211)
- [blackboard_conclusion.go:300-400](file://internal/daemon/blackboard_conclusion.go#L300-L400)

### 停止与完成（Finish）
- Stop
  - 请求关闭 Provider 会话与 Harness，等待退出；若超时则强制停止；最后 settleTaskStopped 确保状态一致
- Finish（operator_finish）
  - 要求 live+idle；先标记 Finish 意图，再关闭会话与等待 Harness 退出
  - 唯一拥有者将 Continuation 置为 completed，校验 ReconciliationCompleted，再置 Task 为 completed
  - 失败回退：settleTaskFailedAfterFinishAbort 保证不残留运行态
- **新增** 助手结论完成路径
  - 自动触发验证流程，通过后直接标记任务为 completed
  - 支持条件性完成：满足特定 Blackboard 状态即视为完成

```mermaid
flowchart TD
S(["收到 Stop/Finish"]) --> Acquire["获取任务控制锁"]
Acquire --> CheckActive{"是否运行/暂停?"}
CheckActive --> |是| CloseProv["关闭 Provider 会话"]
CloseProv --> WaitHarness["等待 Harness 退出"]
WaitHarness --> Settle{"Finish 路径?"}
Settle --> |是| CompleteCont["更新 Continuation 为 completed"]
CompleteCont --> VerifyRecon["校验 ReconciliationCompleted"]
VerifyRecon --> MarkTask["更新 Task 为 completed"]
Settle --> |否| StopSettle["settleTaskStopped"]
CheckActive --> |否| CloseOnly["仅关闭会话/清理"]
CloseOnly --> StopSettle
StopSettle --> AssistedCheck["检查助手结论"]
AssistedCheck --> AutoComplete{"自动完成?"}
AutoComplete --> |是| FinalComplete["最终完成标记"]
AutoComplete --> |否| End(["返回最新任务详情"])
FinalComplete --> End
MarkTask --> End
```

**图表来源**
- [task_handlers.go:1469-1551](file://internal/daemon/task_handlers.go#L1469-L1551)
- [task_handlers.go:1600-1714](file://internal/daemon/task_handlers.go#L1600-L1714)
- [task_handlers.go:1716-1751](file://internal/daemon/task_handlers.go#L1716-L1751)
- [blackboard_conclusion.go:350-450](file://internal/daemon/blackboard_conclusion.go#L350-L450)

**章节来源**
- [task_handlers.go:1469-1551](file://internal/daemon/task_handlers.go#L1469-L1551)
- [task_handlers.go:1600-1714](file://internal/daemon/task_handlers.go#L1600-L1714)
- [task_handlers.go:1716-1751](file://internal/daemon/task_handlers.go#L1716-L1751)
- [blackboard_conclusion.go:350-450](file://internal/daemon/blackboard_conclusion.go#L350-L450)

### 恢复（Resume）与 Continuation 重建
- 两种路径
  - 原生恢复：discoverNativeResumeSession + native resume args（保留会话上下文）
  - 新鲜恢复：从 Goal + 未消费 Harness Steering + 中断 Attempt 检查点 + 最新 Working Snapshot 重建
- 关键步骤
  - prepareFreshResumeContinuation 组装 resumeGoal 与 plan
  - prepareBlackboardV2ContinuationLaunch 创建 Continuation、写入 Launch Pin/Working Snapshot、颁发 Grant
  - launchTaskInBackground 启动新 Continuation，旧 Continuation 在完成时标记 completed
- **新增** 恢复后的助手结论检查
  - 新 Continuation 启动后立即进行助手结论验证
  - 支持增量验证：仅检查变更的部分

```mermaid
sequenceDiagram
participant Client as "客户端"
participant Daemon as "Daemon"
participant TaskSvc as "任务服务"
participant BBV2 as "Blackboard v2 连续性"
participant Harness as "Runtime/Harness"
participant AC as "助手结论服务"
Client->>Daemon : POST /resume [+selection]
Daemon->>TaskSvc : 读取当前选择/未消费指令
Daemon->>Daemon : prepareFreshResumeContinuation
Daemon->>BBV2 : CreateContinuation(Launch Pin/Working Snapshot)
Daemon->>Harness : Launch(new Continuation)
Harness-->>Daemon : lifecycle finish_shutdown
Daemon->>AC : 触发助手结论检查
AC-->>Daemon : 验证结果
Daemon->>TaskSvc : 更新 Continuation/Task 状态
Daemon-->>Client : 202 Accepted
```

**图表来源**
- [task_handlers.go:1816-1912](file://internal/daemon/task_handlers.go#L1816-L1912)
- [task_handlers.go:1963-1987](file://internal/daemon/task_handlers.go#L1963-L1987)
- [continuity.go:764-880](file://internal/blackboardv2/continuity.go#L764-L880)
- [blackboard_conclusion.go:400-500](file://internal/daemon/blackboard_conclusion.go#L400-L500)

**章节来源**
- [task_handlers.go:1816-1912](file://internal/daemon/task_handlers.go#L1816-L1912)
- [task_handlers.go:1963-1987](file://internal/daemon/task_handlers.go#L1963-L1987)
- [continuity.go:764-880](file://internal/blackboardv2/continuity.go#L764-L880)
- [blackboard_conclusion.go:400-500](file://internal/daemon/blackboard_conclusion.go#L400-L500)

### Provider 会话控制与权限响应
- 会话注册表
  - 内存绑定 Task→Session，支持事件回调（只保留白名单字段）
- 能力与模式
  - persistent_session/send_turn/interrupt_then_replace/in_turn_steer/permission_response/resume_session
  - mode 决定操作语义与 outcome 序列（requested→acknowledged→settled/started）
- 权限响应
  - handleProviderPermissionResponse：幂等 key 校验、状态去重、写入 redacted 事件、调用 RespondPermission
  - 失败码映射：timeout/server_closing/session_closed/provider_rejected

```mermaid
classDiagram
class ProviderSessionRegistry {
+bind(taskID, session) error
+get(taskID) (session, bool)
+remove(taskID) session
+closeTask(ctx, taskID) error
+closeAll(ctx) error
}
class ProviderSession {
+SessionID() string
+Capabilities() Capabilities
+SendTurn(ctx, req, emit) Result
+InterruptThenReplace(ctx, req, emit) Result
+SteerInTurn(ctx, req, emit) Result
+RespondPermission(ctx, req, emit) Result
+Close(ctx) error
}
class FakeProviderSession {
+ControlBusy() bool
+SessionClosed() bool
+SessionOffline() bool
+MarkOffline() void
}
ProviderSessionRegistry --> ProviderSession : "管理"
FakeProviderSession ..|> ProviderSession
```

**图表来源**
- [provider_session_control.go:18-111](file://internal/daemon/provider_session_control.go#L18-L111)
- [provider_session.go:140-152](file://internal/runtime/provider_session.go#L140-L152)
- [provider_session.go:261-305](file://internal/runtime/provider_session.go#L261-L305)

**章节来源**
- [provider_session_control.go:18-111](file://internal/daemon/provider_session_control.go#L18-L111)
- [provider_session.go:140-152](file://internal/runtime/provider_session.go#L140-L152)
- [provider_session.go:261-305](file://internal/runtime/provider_session.go#L261-L305)

### Steer 指令队列与中断式转向
- 队列模式（queue）
  - 记录 steering_requested(mode=queue)，可选记录 Runtime Config Version（profile/model/effort 变更）
- 中断模式（active steer）
  - 记录 steering_requested，尝试 native resume；若成功，停止当前 Harness，新建 Continuation，记录 resuming_native 与 steering_applied
- 幂等与冲突
  - native steer 通过 request_id 与 delivery=native_steer 做幂等冲突检测
  - 并发控制：acquireTaskControl/acquireProviderTaskControl 互斥

```mermaid
flowchart TD
Q(["POST /steer/queue"]) --> Record["AppendEvent(steering_requested, mode=queue)"]
Record --> Selection{"包含选择变更?"}
Selection --> |是| SaveCfg["recordSelectedRuntimeConfig"]
Selection --> |否| ReturnQ["返回事件与可选配置版本"]
SaveCfg --> ReturnQ
```

**图表来源**
- [task_handlers.go:2097-2174](file://internal/daemon/task_handlers.go#L2097-L2174)
- [task_handlers.go:2176-2324](file://internal/daemon/task_handlers.go#L2176-L2324)
- [task_handlers.go:2326-2399](file://internal/daemon/task_handlers.go#L2326-L2399)

**章节来源**
- [task_handlers.go:2097-2174](file://internal/daemon/task_handlers.go#L2097-L2174)
- [task_handlers.go:2176-2324](file://internal/daemon/task_handlers.go#L2176-L2324)
- [task_handlers.go:2326-2399](file://internal/daemon/task_handlers.go#L2326-L2399)

### Blackboard v2 连续性：Launch Pin、Working Snapshot 与同步
- Launch Pin：不可变快照（schema/revision/bytes/digest），用于一致性验证
- Working Snapshot：最近已确认的运行时快照，支持原子发布与回滚
- 同步与重放：ClaimTrustedSynchronization/CaptureTrustedSynchronization/SynchronizeContinuation
- 恢复：MaterializeWorkingSnapshot/MaterializeLaunchPin/ActiveSnapshots/RecoverActiveWorkingSnapshots
- **新增** 助手结论数据集成：验证结果、结论报告、审计日志

```mermaid
flowchart TD
Prep["Precommit 投影"] --> TxBegin["开始事务"]
TxBegin --> Project["ProjectRuntimeSnapshotTx"]
Project --> WritePin["写入 Launch Pin"]
WritePin --> WriteState["写入 Working Snapshot 初始状态"]
WriteState --> IssueGrant["颁发 Grant(可选)"]
IssueGrant --> PublishWS["发布 Working Snapshot 到磁盘"]
PublishWS --> Commit["提交事务"]
Commit --> AssistedData["集成助手结论数据"]
AssistedData --> Done(["返回 Continuation/Launch Pin/Token"])
```

**图表来源**
- [continuity.go:764-880](file://internal/blackboardv2/continuity.go#L764-L880)
- [continuity.go:882-976](file://internal/blackboardv2/continuity.go#L882-L976)
- [continuity.go:941-1019](file://internal/blackboardv2/continuity.go#L941-L1019)
- [blackboard_conclusion.go:500-600](file://internal/daemon/blackboard_conclusion.go#L500-L600)

**章节来源**
- [continuity.go:764-880](file://internal/blackboardv2/continuity.go#L764-L880)
- [continuity.go:882-976](file://internal/blackboardv2/continuity.go#L882-L976)
- [continuity.go:941-1019](file://internal/blackboardv2/continuity.go#L941-L1019)
- [blackboard_conclusion.go:500-600](file://internal/daemon/blackboard_conclusion.go#L500-L600)

## 依赖关系分析
- Daemon 对 Task 服务的强依赖：事件追加、状态更新、未消费指令查询
- Daemon 对 Blackboard v2 连续性：Continuation 创建、Working Snapshot 同步、活跃快照恢复
- Daemon 对 Provider 会话：能力探测、操作模式选择、事件回调
- Runtime/Harness 对 Adapter：命令/沙箱封装，统一停止与完成语义
- **新增** Daemon 对助手结论服务：验证规则执行、结果生成、状态更新

```mermaid
graph LR
Daemon["Daemon(task_handlers)"] --> TaskSvc["Task Service"]
Daemon --> BBV2["Blackboard v2 Continuity"]
Daemon --> ProvReg["Provider Session Registry"]
Daemon --> Harness["Runtime/Harness"]
Daemon --> AC["Assisted Conclusion Service"]
Harness --> Adapter["Adapter(Command/Docker)"]
AC --> BBV2
```

**图表来源**
- [task_handlers.go:1-800](file://internal/daemon/task_handlers.go#L1-L800)
- [continuity.go:764-880](file://internal/blackboardv2/continuity.go#L764-L880)
- [provider_session_control.go:18-111](file://internal/daemon/provider_session_control.go#L18-L111)
- [runtime.go:153-189](file://internal/runtime/runtime.go#L153-L189)
- [blackboard_conclusion.go:1-100](file://internal/daemon/blackboard_conclusion.go#L1-L100)

**章节来源**
- [task_handlers.go:1-800](file://internal/daemon/task_handlers.go#L1-L800)
- [continuity.go:764-880](file://internal/blackboardv2/continuity.go#L764-L880)
- [provider_session_control.go:18-111](file://internal/daemon/provider_session_control.go#L18-L111)
- [runtime.go:153-189](file://internal/runtime/runtime.go#L153-L189)
- [blackboard_conclusion.go:1-100](file://internal/daemon/blackboard_conclusion.go#L1-L100)

## 性能与可靠性
- 幂等与冲突
  - Provider 会话操作通过 request_id 与模式约束避免重复执行；冲突返回明确错误码
- 原子性与一致性
  - Continuation 创建采用事务+Launch Pin/Working Snapshot 双保险；失败回滚并恢复磁盘状态
- 健壮性
  - Finish 失败回退至 failed/stopped，避免悬挂运行态
  - 活跃快照恢复确保重启后可继续工作
- **新增** 助手结论可靠性
  - 异步处理避免阻塞主流程
  - 验证失败不影响任务正常完成
  - 支持重试和降级策略

## 故障排查指南
- 常见错误码与原因
  - provider session control conflict：同一会话并发控制冲突
  - session closed：会话已关闭
  - invalid provider session request：缺少 request_id
  - provider session request id is already bound to different content：幂等键冲突
- 定位方法
  - 查看 lifecycle/steering/conversation 事件中的 phase/outcome/mode/request_id
  - 检查 UnconsumedHarnessSteering 是否仍有待消费指令
  - 核对 Continuation 的 ReconciliationCompleted 标记
- **新增** 助手结论故障排查
  - 检查 conclusion_triggered 事件是否产生
  - 查看 validation_passed/validation_failed 事件详情
  - 验证 Blackboard 状态是否符合预期
  - 检查验证规则配置是否正确

**章节来源**
- [provider_session.go:40-75](file://internal/runtime/provider_session.go#L40-L75)
- [task.go:599-627](file://internal/task/task.go#L599-L627)
- [finish_task_resume_test.go:315-351](file://internal/daemon/finish_task_resume_test.go#L315-L351)
- [blackboard_conclusion.go:600-700](file://internal/daemon/blackboard_conclusion.go#L600-L700)

## 结论
本系统以事件驱动为核心，结合 Blackboard v2 连续性保障与 Provider 会话控制，实现了任务从创建到完成的全链路可控、可观测、可恢复。通过明确的幂等与冲突策略、严格的 Finish 所有权与 Reconciliation 校验，系统在复杂交互与异常场景下仍保持高可靠性。

**更新** 新增的助手结论系统进一步增强了任务的自动化处理能力，通过智能验证和自动完成机制，显著提升了系统的智能化水平和用户体验。

## 附录：API参考与使用示例

### 任务相关 API
- 创建任务
  - POST /api/projects/{id}/tasks
  - 输入：goal、runtime_profile_id、model_override、reasoning_effort、runner、run_controls、extras
  - 输出：已创建的任务详情
- 列出任务
  - GET /api/projects/{id}/tasks
- 获取任务详情
  - GET /api/projects/{id}/tasks/{task_id}
- 删除任务
  - DELETE /api/projects/{id}/tasks/{task_id}
- 事件与时间线
  - GET /api/projects/{id}/tasks/{task_id}/events
  - GET /api/projects/{id}/tasks/{task_id}/timeline
  - GET /api/projects/{id}/tasks/{task_id}/transcript
- 停止任务
  - POST /api/projects/{id}/tasks/{task_id}/stop
- 完成任务（operator_finish）
  - POST /api/projects/{id}/tasks/{task_id}/finish
- 恢复任务
  - POST /api/projects/{id}/tasks/{task_id}/resume
  - 可选 body：runtime_profile_id、model_provider_id、model、model_override、reasoning_effort
- 指令（Steer）
  - 队列模式：POST /api/projects/{id}/tasks/{task_id}/steer/queue
    - 输入：directive、可选 selection
    - 输出：event、可选 runtime_config_version
  - 中断模式：POST /api/projects/{id}/tasks/{task_id}/steer
    - 输入：directive、可选 selection
    - 输出：event、可选 runtime_config_version、task（新 Continuation 详情）
- Provider 权限响应
  - POST /api/projects/{id}/tasks/{task_id}/permissions/{permission_id}
    - 输入：request_id、decision(allow/deny)、可选 Idempotency-Key
    - 输出：request_id、permission_request_id、session_id、decision、outcome
- **新增** 助手结论相关 API
  - 获取结论状态：GET /api/projects/{id}/tasks/{task_id}/conclusion
  - 手动触发结论检查：POST /api/projects/{id}/tasks/{task_id}/conclusion/check
  - 获取结论历史：GET /api/projects/{id}/tasks/{task_id}/conclusion/history

**章节来源**
- [task_handlers.go:73-167](file://internal/daemon/task_handlers.go#L73-L167)
- [task_handlers.go:1120-1168](file://internal/daemon/task_handlers.go#L1120-L1168)
- [task_handlers.go:1400-1467](file://internal/daemon/task_handlers.go#L1400-L1467)
- [task_handlers.go:1469-1551](file://internal/daemon/task_handlers.go#L1469-L1551)
- [task_handlers.go:1600-1714](file://internal/daemon/task_handlers.go#L1600-L1714)
- [task_handlers.go:1816-1912](file://internal/daemon/task_handlers.go#L1816-L1912)
- [task_handlers.go:2097-2174](file://internal/daemon/task_handlers.go#L2097-L2174)
- [task_handlers.go:2176-2324](file://internal/daemon/task_handlers.go#L2176-L2324)
- [task_handlers.go:2517-2654](file://internal/daemon/task_handlers.go#L2517-L2654)
- [blackboard_conclusion.go:700-800](file://internal/daemon/blackboard_conclusion.go#L700-L800)

### 实际使用场景示例
- 场景一：首次创建并启动任务
  - 调用创建接口，后端构建 Launch Plan、准备 Continuation、后台 Launch，返回已创建任务详情
- 场景二：向运行中任务发送中断式指令
  - 调用 steer（非 queue），后端记录 steering_requested，尝试 native resume，停止当前 Harness，创建新 Continuation，返回 202 与新任务详情
- 场景三：排队后续指令
  - 调用 steer/queue，后端记录 steering_requested(mode=queue)，如包含选择变更则记录新的 runtime_config_version
- 场景四：恢复已完成/失败的任务
  - 调用 resume，后端优先尝试原生恢复，否则基于 Goal+未消费指令+中断检查点+Working Snapshot 重建 Continuation 并启动
- 场景五：处理 Provider 权限请求
  - 监听 provider_permission_requested 事件，调用权限响应接口，传入 decision 与幂等键，后端写入 redacted 事件并转发给 Provider 会话
- **新增** 场景六：利用助手结论自动完成
  - 任务完成后，系统自动触发助手结论检查
  - 验证 Blackboard 状态，生成结论报告
  - 自动标记任务为 completed，减少人工干预

**章节来源**
- [blackboard_v2_continuity_test.go:538-569](file://internal/daemon/blackboard_v2_continuity_test.go#L538-L569)
- [blackboard_v2_continuity_test.go:203-288](file://internal/daemon/blackboard_v2_continuity_test.go#L203-L288)
- [finish_task_resume_test.go:1134-1166](file://internal/daemon/finish_task_resume_test.go#L1134-L1166)
- [runtime_turn_selection_test.go:141-179](file://internal/daemon/runtime_turn_selection_test.go#L141-L179)
- [assisted_conclusion_test.go:1-100](file://internal/daemon/assisted_conclusion_test.go#L1-L100)
- [assisted_conclusion_control_test.go:1-100](file://internal/daemon/assisted_conclusion_control_test.go#L1-L100)
- [assisted_conclusion_terminal_test.go:1-100](file://internal/daemon/assisted_conclusion_terminal_test.go#L1-L100)