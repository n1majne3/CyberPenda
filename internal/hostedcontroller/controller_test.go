...
func TestHostedControllerStartsOneCodexEvaluationAndOnlyObservesIt(t *testing.T) {
	...
	if strings.Contains(evaluation.Task.Goal, "cyberpenda-blackboard-disabled") {
		t.Fatalf("Hosted Task Goal named a Mode Skill for Disabled Blackboard Mode: %q", evaluation.Task.Goal)
	}
	if !strings.Contains(evaluation.Task.Goal, "REQUIRED SKILL INVOCATION") ||
		!strings.Contains(evaluation.Task.Goal, "`ctf-orchestrator`") {
		t.Fatalf("Hosted Task Goal does not invoke the orchestrator Skill: %q", evaluation.Task.Goal)
	}
