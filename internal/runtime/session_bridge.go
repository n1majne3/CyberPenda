...
type bridgeResponseWaiter struct {
	pending *bridgePending
}

func (w bridgeResponseWaiter) Wait(ctx context.Context) (SandboxBridgeResponse, error) {
	if w.pending == nil {
		return SandboxBridgeResponse{}, ErrSandboxBridgeInvalid
	}
	select {
	case <-w.pending.done:
		return w.pending.result.response, w.pending.result.err
	case <-ctx.Done():
		return SandboxBridgeResponse{}, ctx.Err()
	}
}

func completedBridgeResponseWaiter(completion bridgeCompletion) ProviderSessionResponseWaiter {
	pending := &bridgePending{done: make(chan struct{}), result: completion}
	close(pending.done)
	return bridgeResponseWaiter{pending: pending}
}
...
// BeginSend writes one framed request and returns after the frame is accepted
// by the bridge. The waiter retains the idempotent response even when an
// operation-level caller stops waiting.
func (b *SandboxSessionBridge) BeginSend(request SandboxBridgeRequest) (ProviderSessionResponseWaiter, error) {
	request.JSONRPC = "2.0"
	...
	if b.state != "running" {
		b.mu.Unlock()
		return nil, ErrSandboxBridgeClosed
	}
	...
	b.writeMu.Unlock()
	if err != nil {
		writeErr := fmt.Errorf("write sandbox bridge request: %w", err)
