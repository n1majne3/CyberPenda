...
func (s *Service) Status(ctx context.Context,c owner.Contract)(Status,error) {
 ...
 kind,id,err:=boardIdentity(c);if err!=nil {return result,err}
 where:=`owner_kind='session' AND owner_id=?`
 if kind=="project" { where=`owner_kind='task' AND owner_id IN (SELECT id FROM tasks WHERE project_id=?)` }
 ...
 if err!=nil {return result,err};defer rows.Close()
 for rows.Next() {var raw string; if err=rows.Scan(&raw);err!=nil {return result,err};var r Receipt;if err=json.Unmarshal([]byte(raw),&r);err!=nil {return result,err};result.Receipts=append(result.Receipts,r)}
