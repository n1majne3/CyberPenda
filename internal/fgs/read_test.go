...
func TestDeliveryStatusTracksRejectionAndWithdrawal(t *testing.T) {
	...
	if err != nil || status.ActionRequired != 1 {
		t.Fatalf("rejected: %+v %v", status, err)
	}
	...
	status, err = s.Status(t.Context(), c)
	if err != nil || status.ActionRequired != 0 || len(status.Receipts) != 2 || status.LastAcceptedAt == "" {
		t.Fatalf("withdrawn: %+v %v", status, err)
	}
}
...
func TestStepPriorityAndFactDataReferencesSurviveAcceptance(t *testing.T) {
	...
	history, err := s.History(t.Context(), c, "step:b")
	if err != nil || history[0].Priority != "high" || history[0].Executor != "execute-1" {
		t.Fatalf("step: %+v %v", history, err)
	}
	history, err = s.History(t.Context(), c, "fact:c")
	if err != nil || len(history[0].DataRefs) != 1 || history[0].AcceptedAt == "" {
		t.Fatalf("fact: %+v %v", history, err)
	}
}
...
func TestHistoryPagesReturnNewestVersionsFirst(t *testing.T) {
	...
	if err != nil || len(page) != 1 || page[0].Version != 2 {
		t.Fatalf("newest: %+v %v", page, err)
	}
	page, err = s.HistoryPage(t.Context(), c, "goal:a", 2, 1)
	if err != nil || len(page) != 1 || page[0].Version != 1 {
