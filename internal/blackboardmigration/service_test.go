...
func openLegacyFixtureStore(t *testing.T, databasePath string) *store.DB {
	...
	installLegacyWorkflowStateFixture(t, fresh)
...
// installLegacyWorkflowStateFixture restores v1-only Finish state after the
// fresh-store bootstrap has applied migration 33. Migration-source tests use
// this explicit fixture to model the pre-cutover database decoded offline.
