...
func (service *Service) Evaluate(ctx context.Context, projectID, taskID string) (Readiness, error) {
	...
	queries := []struct {
		...
		{BlockerOpenAttempts, "Task-owned Blackboard Attempts are still open.", "blackboard", `SELECT COUNT(*) FROM blackboard_v2_attempt_origins origins JOIN task_continuations continuations ON continuations.id=origins.continuation_id JOIN blackboard_v2_records records ON records.project_id=origins.project_id AND records.key=origins.key WHERE continuations.task_id=? AND records.type='attempt' AND json_extract(records.record_json,'$.status')='open' AND NOT EXISTS (SELECT 1 FROM challenge_attempts history WHERE history.project_id=records.project_id AND history.attempt_key=records.key)`, []any{taskID}, true},
	}
