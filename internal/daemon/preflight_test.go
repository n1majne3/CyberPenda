...
func TestSessionPreflightUsesGlobalLaunchInputsWithoutProjectScope(t *testing.T) {
	server := newDaemon(t)
	t.Setenv("CODEX_API_KEY", "configured-secret")
	profileID := createRuntimeProfile(t, server, `{
		"name":"Codex",
	...
	body := []byte(`{"runtime_profile_id":"` + profileID + `","runner":"sandbox","reasoning_effort":"high"}`)
	req := httptest.NewRequest(http.MethodPost, "/api/sessions/preflight", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	resp := httptest.NewRecorder()
	server.ServeHTTP(resp, req)

	if resp.Code != http.StatusOK {
		t.Fatalf("expected Session preflight status 200, got %d with body %s", resp.Code, resp.Body.String())
	}
	var result struct {
		Pass   bool `json:"pass"`
		Checks []struct {
	...
	if !result.Pass {
		t.Fatalf("expected Session preflight to pass, got %#v", result.Checks)
	}
	if !checkNamed(result.Checks, "credentials", "pass") {
		t.Fatalf("expected global credentials check to pass, got %#v", result.Checks)
	}
}
...
func TestPreflightBuiltinSkillPreviewUsesSourceFreeID(t *testing.T) {
	...
	projectID := createProject(t, server, `{"name":"Acme","scope":{"domains":["example.com"]}}`)
	profileID := createRuntimeProfile(t, server, `{"name":"Fake","provider":"fake"}`)
	putSkill(t, server, "cyberstrikeai-tooling-nmap", `{
	...
	if len(result.Skills) != 1 || result.Skills[0].ID != "tooling-nmap" || result.Skills[0].Name != "tooling-nmap" {
