...
func TestBuildTaskAdapterAppliesSandboxVPNTunFromRunControls(t *testing.T) {
	server, err := NewServer(Config{
		Version:              "test",
		DBPath:               filepath.Join(t.TempDir(), "pentest.db"),
		RuntimeRoot:          t.TempDir(),
	...
	if !strings.Contains(joined, "--device /dev/net/tun") {
		t.Fatalf("expected /dev/net/tun device in launch args, got %v", args)
	}
	if !strings.Contains(joined, "--cap-add NET_ADMIN") {
		t.Fatalf("expected NET_ADMIN capability in launch args, got %v", args)
	}
}
