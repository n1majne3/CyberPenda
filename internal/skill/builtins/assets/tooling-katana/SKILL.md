...
For a JS-only sweep of a domain, run katana itself with `-jc` (JS crawl)
and grep the collected scripts for endpoints, keys, and secrets; no other
crawler or JS extractor is preinstalled in the sandbox.
