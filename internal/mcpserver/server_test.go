...
func connectMCPV2(t *testing.T, deps mcpserver.V2Deps) *sdkmcp.ClientSession {
	t.Helper()
	ctx := context.Background()
	server := mcpserver.NewV2(deps)
	t1, t2 := sdkmcp.NewInMemoryTransports()
	if _, err := server.Connect(ctx, t1, nil); err != nil {
		t.Fatalf("connect v2 server: %v", err)
	}
	client := sdkmcp.NewClient(&sdkmcp.Implementation{Name: "test-client-v2", Version: "test"}, nil)
	session, err := client.Connect(ctx, t2, nil)
	if err != nil {
		t.Fatalf("connect v2 client: %v", err)
	}
	t.Cleanup(func() { _ = session.Close() })
	return session
}
...
func TestBlackboardChangeMCPSchemaAdvertisesObjectiveAndAttemptCreateEnvelope(t *testing.T) {
	// Ported from the user-authored v1 blackboard_apply schema regression: the
	// advertised trusted MCP input schema must guide create of objectives and
	// attempts through the accepted v2 change envelope (not property_map/kind).
	session := connectMCPV2(t, mcpserver.V2Deps{
