...
const (
	...
	// ProviderHermes identifies retained history from the retired Hermes Runtime.
...
func (s *Service) Create(name string, provider Provider, fields Fields) (Profile, error) {
	return s.create(name, provider, fields)
}

// CreateTx validates and stores a Runtime Profile in the caller's transaction.
// It is used when Profile identity and related advanced configuration must be
// committed atomically.
func (s *Service) CreateTx(tx *sql.Tx, name string, provider Provider, fields Fields) (Profile, error) {
	if tx == nil {
		return Profile{}, errors.New("runtime profile transaction is required")
	}
	created, fieldsJSON, err := s.prepareCreate(name, provider, fields)
	if err != nil {
		return Profile{}, err
	}
	if _, err := tx.Exec(
		`INSERT INTO runtime_profiles (id, name, provider, fields_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)`,
		created.ID, created.Name, string(created.Provider), fieldsJSON,
...
func (s *Service) prepareCreate(name string, provider Provider, fields Fields) (Profile, string, error) {
	...
	return created, string(fieldsJSON), nil
}
